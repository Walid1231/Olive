...
fun FolderSongsScreen(
    ...
    val folderSongs = allSongs.filter { it.folder == folderPath }
    val gc = LocalGhibliColors.current

    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(folderPath, color = gc.textPrimary) },
                navigationIcon = {
        ...
        LazyColumn(
            ...
            items(folderSongs, key = { it.id }) { song ->
                SongListItem(
                    song = song,
                    onClick = { onSongClick(folderSongs, folderSongs.indexOf(song)) },
