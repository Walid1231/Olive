...
fun AlbumDetailScreen(
    ...
    val albumSongs = allSongs.filter { it.album == albumName && it.artist == artistName }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(albumName, color = gc.textPrimary) },
                navigationIcon = {
        ...
        LazyColumn(
            ...
            items(albumSongs, key = { it.id }) { song ->
                SongListItem(
                    song = song,
                    onClick = { onSongClick(albumSongs, albumSongs.indexOf(song)) },
